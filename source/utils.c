...
int parse_param_sfo(char * file, char *title_name)
{
    ...
    if(!sysLv2FsOpen(file, 0, &fd, S_IRWXU | S_IRWXG | S_IRWXO, NULL, 0))
    {
        ...
        while(str<len)
        {
            ...
            if(!strcmp((char *) &mem[str], "CATEGORY")) {
                strncpy(title_name, (char *) &mem[pos], 63);
                ct++;
...
{
    return ((valore >> 24) | ((valore<<8) & 0x00FF0000) | ((valore>>8) & 0x0000FF00) | (valore << 24));
}
...
u16 find_table( u16 offset, const char *path )
{
	...
	sysLv2FsOpen( path, SYS_O_RDONLY, &file, 0, NULL, 0 );
	if( file == -1 )
		return 0;
		
	sysLv2FsLSeek64( file, offset, 0, &pos ); 
	sysLv2FsRead( file, &result, sizeof( u16 ), &read );
	sysLv2FsClose( file );
...
int is_file_exist( char* path )
{
	...
	if( sysLv2FsStat(path, &stat) == 0 ) return 1;
...
{
//Trovo numero di parametri
	u16 num_param;
	
	num_param = find_table( ENTRY_NUM, path_sfo );
	num_param = swap_word( num_param );
	if( num_param == 0 )
		return -1;
...
//Trovo inizio data
	...
	data_start = find_table( DATA_TABLE, path_sfo );
	data_start = swap_word( data_start );
	
//Trovo offset TITLE_ID
	u16 offset_id;
	u16 tmp;
	
	tmp = START_DEF_TABLE + ( 16 * num_param - 4 );
	offset_id = find_table( tmp, path_sfo );
	offset_id = swap_word( offset_id );
	offset_id = offset_id + data_start;

//Memorizzo TITLE_ID
	int fd;
	u64 pos, read;
	
	sysLv2FsOpen( path_sfo, SYS_O_RDWR, &fd, 0, NULL, 0 ); //Apro SFO
	sysLv2FsLSeek64( fd, offset_id, 0, &pos ); //Mi posiziono dove inizia il TITLE_ID
	sysLv2FsRead( fd, id, 10, &read );
	sysLv2FsClose( fd );
...
int compare_fw_ver( char *self )
{
	...
	ret = sysLv2FsOpen( self, SYS_O_RDWR, &fd, 0, NULL, 0 );
	if( ret != 0 ) return -4; //file not exist
	
	sysLv2FsLSeek64( fd, 0x42E, 0, &pos );
	
	ret = sysLv2FsRead( fd, &ver_read, 0x2, &readed );
	if( ret != 0 ) return -3; //Error reading self
	
	if( fw_ver < ver_read && firmware < 0x421C )
		return -2; //This game can not be booted, rebuild EBOOT.BIN, .SELF, .SPRX with true ancestor or similar
		
	else if( fw_ver < ver_read && firmware >= 0x421C )
		return -1; //The fw version in the game should be fixed, Fix it!
		
	else
		return 0; //Nothing to report X)
}
*/
...
void patch_error_09( const char *path )
{
	...
	//Vedo se "entry" è un self, un sprx, un eboot o una cartella
	...
	//FILE
		...
		//SPRX
			...
			//Azzero array
				memset( path_sprx, 0, sizeof( char ) * 1000 );
				
			//Copio percorso e nome file in path_sprx
				snprintf( path_sprx, 1000, "%s/%s", path, d_name );
				
			//Apro self e modifico la versione del fw	
				ret = sysLv2FsOpen( path_sprx, SYS_O_RDWR, &sprx, 0, NULL, 0 );
				sysLv2FsLSeek64( sprx, OFF_FW_VER_SPRX, 0, &pos );
				
			//Vedo se il fw è maggiore di 4.21
				ret = sysLv2FsRead( sprx, &ver, 0x2, &readed );
				if( ver > fw_ver )
				{
				//Scrivo valore 3.41
					sysLv2FsLSeek64( sprx, OFF_FW_VER_SPRX, 0, &pos );
					ret = sysLv2FsWrite( sprx, &fw_420, 0x2, &written );
				}
				sysLv2FsClose( sprx );
			}
			
		//SELF & EBOOT
			else if(( entry.d_name[ ext ] == 's' && entry.d_name[ ext + 1 ] == 'e' && entry.d_name[ ext + 2 ] == 'l' && entry.d_name[ ext + 3 ] == 'f') ||
			...
			//Azzero array
				memset( path_self, 0, sizeof( char ) * 1000 );	
				
			//Copio percorso e nome file in path_sprx
				snprintf( path_self, 1000, "%s/%s", path, d_name );
				
			//Apro lo self e modifico la versione del fw	
				ret = sysLv2FsOpen( path_self, SYS_O_RDWR, &self, 0, NULL, 0 );
				sysLv2FsLSeek64( self, OFF_FW_VER, 0, &pos );
				
			//Vedo se il fw è maggiore di 4.21
				ret = sysLv2FsRead( self, &ver, 0x2, &readed );
				if( ver > fw_ver )
				{
				//Scrivo valore 3.41
					sysLv2FsLSeek64( self, OFF_FW_VER, 0, &pos );
					ret = sysLv2FsWrite( self, &fw_420, 0x2, &written );
				}
				sysLv2FsClose( self );
			}
		}
	...
	//DIRECTORY
		else if( entry.d_type == 1 && strcmp( entry.d_name, "." ) != 0 && strcmp( entry.d_name, ".." ) != 0 && strcmp( entry.d_name, "GAMES" ) != 0 && strcmp( entry.d_name, "GAMEZ" ) != 0)   //aggiungere iris, multiman, nostro manager (PS3ITA Manager), showtime, altro??
		{
			char path_dir[ 1000 ];
 
			snprintf( path_dir, 1000, "%s/%s", path, d_name);
...
void copy_usb_to_PS3ITA(char * path)
{
    ...
    n+= 47;
    sprintf(name, "/dev_usb00%c/PS3ITA", n);
    ...
    sprintf(filename, "/dev_usb/PS3ITA %s PS3ITA Manager?\nVol: %1.2f MB", 
                        language[GLUTIL_WTO], 
    ...
    if(dialog_action == 1) {
        ...
        DPrintf("%s %s\n %s %s\n\n", language[GAMECPYSL_STARTED], "USB", language[GLUTIL_WTO], "PS3ITA Manager");
