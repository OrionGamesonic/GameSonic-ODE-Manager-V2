...
void client_thread(void *conn_s_p)
{
	...
	while(appstate != 1 && (bytes = recv(conn_s, temp, 511, 0)) > 0)
	{
		...
		if(strcmp2(cmd, "FEAT") == 0)
		{
			char *feat[] =
			{
				"REST STREAM", "PASV", "PORT", "MDTM", "MLSD", "SIZE", "SITE CHMOD", "SITE RESTART", "SITE SHUTDOWN", "APPE",
		...
		if(authorized == 1)
		{
			...
			if(strcmp2(cmd, "SITE") == 0)
			{
				if(itemp == 1)
				{
					...
					else if((strcmp2(cmd, "QUITMM") == 0) || (strcmp2(cmd, "EXITAPP") == 0))
					{
						appstate = 1;
					...
					else if((strcmp2(cmd, "RESTART") == 0) || (strcmp2(cmd, "REBOOT") == 0))
					{
						appstate = 1;
						
						bytes = ftpresp(temp, 221, "Rebooting...");
						sysProcessExitSpawn2( "/dev_hdd0/temp", NULL, NULL, NULL, 0, 1000, SYS_PROCESS_SPAWN_STACK_SIZE_32K);
						exit(0);
					}
					else if(strcmp2(cmd, "SHUTDOWN") == 0)
					{
						appstate = 1;
